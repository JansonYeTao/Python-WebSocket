...
```bash
uvicorn server.listen:app --host 0.0.0.0 --port 8000 --reload
```
...
.
├── server/
│   └── listen.py        # FastAPI app + WebSocket endpoint + simple browser page
├── clients/
...
Before implementing a full from-scratch tutorial, use the benchmark plan in `benchmarks/PLAN.md` to measure when WebSocket is beneficial versus overkill.

- Plan file: `benchmarks/PLAN.md`
...
```bash
uvicorn server.listen:app --host 0.0.0.0 --port 8000
```

In a second terminal, run the benchmark script:

```bash
python benchmarks/run_benchmark.py --base-url http://127.0.0.1:8000 --clients 20 --messages 20 --payload-size 128 --transport both
```

This produces JSON results at `benchmarks/results/latest.json` with:
