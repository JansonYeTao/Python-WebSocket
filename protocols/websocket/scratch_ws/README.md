...
```bash
python protocols/websocket/scratch_ws/handshake_server.py
```
...
```bash
python protocols/websocket/scratch_ws/frame_echo_server.py
```
...
Use this to understand protocol mechanics, then compare with `protocols/websocket/server/listen.py` for framework abstraction.
